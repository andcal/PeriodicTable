	...
	renderAnimation() {
		if(!this.state.ineteraction) {
			return(
				...
				<svg version="1.1" id="table" x="0px" y="0px" width="960px" height="560px" viewBox="0 0 960 560">
					...
					>
						...
						<rect id={styles.alkalinGroup} x="42" y="60" width="38" height="0" fill="#FFFF00"/>
					
						<rect id={styles.halogenGroup} x="716" y="61"  width="38" height="0" fill="#FFFF00"/>
					...
					<svg>
						<text transform="matrix(1 0 0 1 36 416.04)">
							<tspan x="0" dy="1.2em">The name ‘halogen’ means ‘salt-producing’. The elements in column 17 belong to this group.</tspan>
						 	<tspan x="0" dy="1.2em">When halogens react with elements in the first family, called alkalins, they produce a wide range of salts.</tspan>
						</text>
					</svg>
		...
		if(this.state. ineteraction) {
			...
			return(
				<div className={styles.interactionContainer}>
					...
					<div>
						<p>{this.state.dropAlkName}</p>
						<p>{this.state.dropHalName}</p>
					</div>
					...
					>
						<div className={styles.mixAlk}>{this.state.dropAlkSymbol}</div>
					</Droppable>
					...
					>
						<div className={styles.mixHal}>{this.state.dropHalSymbol}</div>
					</Droppable>
